...
PARAMS_TO_FIND = ["search", "ordering", "page", "page_size"]
...
def check_file(filepath):
    ...
    for param in PARAMS_TO_FIND:
        ...
        swagger_params = re.findall(
            rf'openapi\.Parameter\(\s*[\'"]({param})[\'"]', content, re.IGNORECASE
        )
        ...
        if swagger_params and has_filter_backend:
            results.append(
                f"⚠️ Potential duplication of '{param}' parameter in {filepath}"
            )
