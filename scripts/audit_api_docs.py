...
def check_file(filepath):
    ...
    if "APIView" in content or "ViewSet" in content:
        ...
        for class_match in class_matches:
            ...
            if (
                "APIView" not in parent_classes
                and "ViewSet" not in parent_classes
                and "GenericAPIView" not in parent_classes
            ):
                continue
            ...
            if (
                "swagger_auto_schema" not in class_content
                and "APIView" in parent_classes
            ):
            ...
            if (
                "ViewSet" in parent_classes
                and "QueueMeViewSet" not in parent_classes
                and "test" not in filepath.lower()
            ):
                issues_found["not_using_base_viewset"].append(
                    f"{filepath} - {class_name}"
                )

            # Check for dedupe_manual_parameters usage
            if (
                "manual_parameters" in class_content
                and "dedupe_manual_parameters" not in class_content
            ):
            ...
            for param, count in param_counts.items():
                if count > 1:
                    issues_found["duplicate_params"].append(
                        f"{filepath} - {class_name} - {param} ({count} times)"
                    )
            ...
            if re.search(SWAGGER_DECORATOR_PATTERN, class_content):
                ...
                for swagger_match in swagger_matches:
                    ...
                    if "responses=" not in deco_content:
                        issues_found["missing_responses"].append(
                            f"{filepath} - {class_name}"
                        )
...
def fix_issues(filepath):
    ...
    if "manual_parameters=[" in content and "dedupe_manual_parameters([" not in content:
        content = content.replace(
            "manual_parameters=[", "manual_parameters=dedupe_manual_parameters(["
        )
        # Fix closing parenthesis
        content = re.sub(
            r"manual_parameters=dedupe_manual_parameters\(\[(.*?)\]\)",
...
def main():
    ...
    if issues_found["duplicate_params"]:
        print(
            "  - Remove duplicate parameter definitions and use centralized parameters"
        )
