...
(function() {
    ...
    const util = {
      ...
      formatDate(dateString, locale = QueueMe.lang.current) {
        ...
        const options = config.dateTimeFormat[locale]?.date || config.dateTimeFormat.en.date;
      ...
      formatTime(timeString, locale = QueueMe.lang.current) {
        ...
        const options = config.dateTimeFormat[locale]?.time || config.dateTimeFormat.en.time;
      ...
      formatDateTime(dateTimeString, locale = QueueMe.lang.current) {
        ...
        const dateOptions = config.dateTimeFormat[locale]?.date || config.dateTimeFormat.en.date;
        const timeOptions = config.dateTimeFormat[locale]?.time || config.dateTimeFormat.en.time;
      ...
      showToast(message, type = 'info', duration = config.toastDuration) {
        const toastContainer = this.$('.toast-container') || this._createToastContainer();
      ...
      validateForm(formElement) {
        const form = typeof formElement === 'string' ? this.$(formElement) : formElement;
    ...
    const auth = {
      ...
      async login(credentials) {
        try {
          const response = await util.post(`${config.apiBaseUrl}/auth/login/`, credentials);
      ...
      async verifyOTP(payload) {
        try {
          const response = await util.post(`${config.apiBaseUrl}/auth/verify_otp/`, payload);
      ...
      updateAuthUI(user) {
        ...
        if (user) {
          ...
          authContainer.innerHTML = `
            <div class="user-info">
              <span class="user-name">${user.user_type === 'customer' ? 'Customer' : 'Staff'}</span>
    ...
    const lang = {
      ...
      init() {
        ...
        util.delegate('body', 'click', '.js-lang-switch', this.handleLanguageSwitch.bind(this));
      },
    ...
    const socket = {
      ...
      send(channel, data) {
        ...
        if (!connection || connection.status !== 'open') {
          console.error(`Cannot send message: WebSocket for ${channel} is not open`);
          return false;
        }
      ...
      on(channel, event, callback) {
        if (!this.connections[channel]) {
          console.error(`Cannot add listener: WebSocket for ${channel} does not exist`);
          return;
        }
      ...
      off(channel, event, callback) {
        if (!this.connections[channel] || !this.connections[channel].listeners[event]) {
          return;
        }
        ...
        if (callback) {
          ...
          this.connections[channel].listeners[event] = 
            this.connections[channel].listeners[event].filter(cb => cb !== callback);
        } else {
      ...
      _onClose(channel, event) {
        console.log(`WebSocket connection closed for ${channel}:`, event.code, event.reason);
      ...
      _attemptReconnect(channel) {
        ...
        if (connection.reconnectAttempts <= config.maxRetryAttempts) {
          console.log(`Attempting to reconnect to ${channel} (${connection.reconnectAttempts}/${config.maxRetryAttempts})...`);
          ...
          console.error(`Failed to reconnect to ${channel} after ${config.maxRetryAttempts} attempts.`);
        }
      },
      ...
      _triggerListeners(channel, event, data) {
        ...
        connection.listeners[event].forEach(callback => {
          try {
            ...
            console.error(`Error in WebSocket listener for ${channel} event ${event}:`, error);
          }
        });
      }
    };
    ...
    function handleAjaxForm(event) {
      ...
      // Make AJAX request
      const requestFn = method === 'GET' ? util.get : 
                       method === 'POST' ? util.post : 
                       method === 'PUT' ? util.put : util.delete;
      ...
      requestFn(url, data)
        .then(response => {
          ...
          if (response.ok) {
            ...
            // Show error message
            const errorMessage = response.data?.detail || 'An error occurred. Please try again.';
        ...
        .catch(error => {
          // Show error message
          util.showToast('An unexpected error occurred. Please try again later.', 'error');
    ...
    function handleToggle(event) {
      ...
      if (trigger.dataset.showText && trigger.dataset.hideText) {
        ...
        trigger.textContent = isVisible ? trigger.dataset.hideText : trigger.dataset.showText;
      }
    }
