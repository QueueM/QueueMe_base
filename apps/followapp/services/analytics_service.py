...
class FollowAnalyticsService:
    ...
    def calculate_retention_rate(shop) -> Dict[str, float]:
        """
        Calculate follower retention rates.
        Shows what percentage of followers remain after different periods.

        Args:
            shop: The shop object

        Returns:
            Dict with retention rates for different periods
        """
        # Get all follow events for this shop
        follow_events = FollowEvent.objects.filter(shop=shop).order_by(
            "customer", "timestamp"
        )
