...
class ContentModerationService:
    ...
    MALICIOUS_EXTENSIONS = {
        '.exe', '.dll', '.bat', '.cmd', '.vbs', '.js', '.jar', '.sh',
        '.py', '.php', '.asp', '.aspx', '.jsp', '.cgi', '.pl', '.msi'
    }
    ...
    def is_inappropriate(cls, text: str) -> bool:
        ...
        # Check cache first
        cache_key = f"inappropriate_text:{hashlib.md5(normalized_text.encode()).hexdigest()}"
        ...
        if hasattr(settings, 'CONTENT_MODERATION_API_ENABLED') and settings.CONTENT_MODERATION_API_ENABLED:
            try:
    ...
    def _calculate_spam_score(title: str, content: str) -> int:
        ...
        promotional_keywords = ["buy", "free", "offer", "discount", "sale", "cheap", "deal", "price", 
                              "limited time", "exclusive", "guarantee", "amazing", "incredible"]
    ...
    def _analyze_sentiment(text: str) -> float:
        ...
        negative_words = ["bad", "poor", "terrible", "horrible", "awful", "dreadful", "disappointing", 
                         "hate", "dislike", "unhappy", "frustrated", "angry", "worst"]
