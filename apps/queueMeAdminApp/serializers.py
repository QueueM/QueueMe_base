...
class SupportMessageSerializer(serializers.ModelSerializer):
    ...
    def validate_sender_id(self, value):
        ...
        if value != str(request.user.id) and not request.user.is_staff:
            raise serializers.ValidationError(_("You cannot specify a different sender."))
    ...
    def validate(self, data):
        if "start_time" in data and "end_time" in data:
            ...
            if data["start_time"] < serializers.DateTimeField().now:
                raise serializers.ValidationError(_("Maintenance cannot be scheduled in the past."))
