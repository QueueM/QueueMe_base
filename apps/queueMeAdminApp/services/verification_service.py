...
class VerificationService:
    ...
    def create_verification_request(shop, documents=None):
        ...
        existing = VerificationRequest.objects.filter(shop=shop, status="pending").first()

        if existing:
            return existing

        # Create new request
        request = VerificationRequest.objects.create(shop=shop, documents=documents or [])
    ...
    @staticmethod
    @transaction.atomic
    def reject_verification(verification_request, admin_user, rejection_reason, notes=""):
        """
        ...
        # Send notification to shop owner/manager
        VerificationService._send_verification_notifications(verification_request, False)
    ...
    def get_pending_verifications():
        ...
        """
        return VerificationRequest.objects.filter(status="pending").order_by("submitted_at")
    ...
    def get_verification_stats():
        ...
        if approved_requests.exists():
            ...
            avg_time_seconds = approved_requests.annotate(time_diff=time_diff).aggregate(
                avg_time=Avg("time_diff")
            )["avg_time"]

            if avg_time_seconds:
                avg_verification_time = avg_time_seconds.total_seconds() / 3600  # Convert to hours
