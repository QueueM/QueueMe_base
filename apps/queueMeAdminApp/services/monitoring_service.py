...
class MonitoringService:
    ...
    def get_overall_status():
        ...
        has_major_outage = component_statuses.filter(status=PLATFORM_STATUS_MAJOR_OUTAGE).exists()
        has_partial_outage = component_statuses.filter(
            status=PLATFORM_STATUS_PARTIAL_OUTAGE
        ).exists()
        has_degraded = component_statuses.filter(status=PLATFORM_STATUS_DEGRADED).exists()
