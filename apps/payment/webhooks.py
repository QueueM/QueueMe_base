...
class MoyasarWebhookView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not MoyasarService.verify_webhook_signature(request.body, signature):
            logger.error("Invalid webhook signature")
            return HttpResponse(status=401)

        # Log webhook event
        event_type = webhook_data.get("type")
        logger.info(f"Received Moyasar webhook: {event_type}")
        ...
        if event_type in MOYASAR_WEBHOOK_EVENTS:
            try:
                ...
                PaymentLog.objects.create(
                    action=f"webhook_{event_type}", details=webhook_data
                )
