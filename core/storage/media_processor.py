...
This module provides utilities for processing and optimizing image and video files
before storage.
"""
...
class MediaProcessor:
            ...
            if len(info) >= 2:
                ...
                if size_mb > max_size_mb:
                    ...
                    if hasattr(video_file, "name"):
                        processed_video.name = video_file.name
                    else:
    ...
    def validate_video(
        ...
        try:
            # Check format
            if hasattr(video_file, "name"):
                file_ext = os.path.splitext(video_file.name)[1].lower().lstrip(".")
                if allowed_formats and file_ext not in allowed_formats:
                    raise MediaProcessingException(
            ...
            # Check if ffmpeg is available for duration check
            try:
                ...
                logger.warning("ffprobe not available, skipping duration check")
