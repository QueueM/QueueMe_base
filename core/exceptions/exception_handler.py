...
from rest_framework.exceptions import ValidationError, NotAuthenticated, NotFound, APIException
...
def get_error_details(exception: Exception) -> Optional[Dict[str, Any]]:
    ...
    if isinstance(exception, ValidationError) and hasattr(exception, "detail") and not isinstance(exception.detail, str):
