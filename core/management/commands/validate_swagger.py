    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            generator = OpenAPISchemaGenerator(
                info=info, patterns=get_resolver().url_patterns
            )
            ...
            try:
                ...
                if not schema:
                    self.stdout.write(
                        self.style.ERROR("Schema generation returned empty schema")
                    )
                    return
                ...
                try:
                    ...
                    self.stdout.write(
                        f"Found {endpoints_count} API endpoints in the schema"
                    )
                    ...
                    self.stdout.write(
                        f"Found {params_count} parameters across all endpoints"
                    )
                    ...
                    if duplicate_params:
                        ...
                        for path, method, param in duplicate_params[:5]:
                            self.stdout.write(
                                f"  - {path} [{method}]: {param[0]} (in {param[1]})"
                            )
                        if len(duplicate_params) > 5:
                            self.stdout.write(
                                f"  - ... and {len(duplicate_params) - 5} more"
                            )
                    else:
                        self.stdout.write(
                            self.style.SUCCESS(
                                "No duplicate parameters found in the schema"
                            )
                        )
                    ...
                    self.stdout.write(
                        self.style.ERROR(f"Error processing schema: {str(e)}")
                    )
