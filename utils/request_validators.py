...
def validate_data(data: Dict, schema: Dict) -> Dict:
    ...
    for field, rules in schema.items():
        ...
        if min_length is not None and isinstance(field_value, str) and len(field_value) < min_length:
        ...
        if min_value is not None and isinstance(field_value, (int, float)) and field_value < min_value:
            field_errors.append(f'Must be at least {min_value}')
        
        # Validate maximum value
        max_value = rules.get('max_value')
        if max_value is not None and isinstance(field_value, (int, float)) and field_value > max_value:
            field_errors.append(f'Must be at most {max_value}')
        
        # Validate regex pattern
        pattern = rules.get('pattern')
        if pattern is not None and isinstance(field_value, str) and not re.match(pattern, field_value):
